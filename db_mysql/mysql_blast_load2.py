...
errorlogfile = os.path.basename(sys.argv[0])+"_errorlog.txt"
...
if not cursor.execute("SHOW TABLES LIKE 'blast';"):
	...
	cursor.execute("CREATE TABLE `blast` (\
	  ...
	  PRIMARY KEY (`bitscore`,`q_seqid`,`h_seqid`,`q_start`),\
	  KEY `q_org` (`q_org`),\
...
def process_lines(file_lines, filename):
	...
	for line in file_lines:
		...
		values = ( filename, q_seqid, q_org, q_seqkey, q_tail , h_seqid, h_org, h_seqkey, h_tail, pident, q_len, q_start, q_end, h_len, h_start, h_end, evalue, bitscore , loadstamp)
		...
		if args.action == "load" and ( counter == 5000 or totalcounter == len( file_lines ) ):
			...
			try:	
				cursor.executemany( "REPLACE INTO blast ( filename, q_seqid, q_org, q_seqkey, q_tail ,\
